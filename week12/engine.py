...
class Value:
    ...
    def __add__(self, other): # f=x+y
        ...
        def _backward():
            self.grad += out.grad  # gx = gf #【mynote】因為gx = gf，所以x跟y的grad都要加gf，而且是累加
    ...
    def relu(self):
        out = Value(0 if self.data < 0 else self.data, (self,), 'ReLU') #【mynote】如果data小於0就等於0，否則就是data本身
    ...
    def backward(self):
        ...
        # topological order all of the children in the graph #【mynote】算出正向計算的順序存在topo
        topo = []
        visited = set()
        def build_topo(v):
            if v not in visited:
        ...
        self.grad = 1
        for v in reversed(topo): #【mynote】將topo順序倒過來計算
            v._backward()
