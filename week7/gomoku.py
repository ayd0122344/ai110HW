...
attackScores = [0, 3, 10, 30, 100, 500]#【mynote】趕快贏比較重要，所以攻擊方分數略高於防守方
guardScores = [0, 2, 9, 25, 90, 400]
attack = 1
guard = 2
#【mynote】電腦會算出目前盤面上所有可以下的位置中哪一個分數最高，就下那一個位置
#【mynote】要提升棋力要用到MiniMax演算法來做對局搜尋，預判後4步可能會出現的下法，並使攻擊防守在每一階交替，所以要找出攻擊得最多分(找最大)，防守損失最少分的位置(找最小)
#【mynote】MiniMax演算法改進後變成alpha-beta修剪法，把不可能改變結果的分枝修剪掉，減少分之數量、降低搜尋空間、提高搜尋效率
#【mynote】alpha-beta修剪法，假設目前階層要取小，比最小值大的分支就不用看了，反之亦然
def getScore(board, r, c, turn, mode): #【mynote】檢查連幾子
    score = 0
