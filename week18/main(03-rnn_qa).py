...
def q2a(q):
    ...
    with torch.no_grad():
        ...
        for word in q: #【mynote】1. 強制q用這模型去輸入這些字，然後先不管他的輸出
            output, state = model(input, state)
            word_id = corpus.dictionary.word2idx[word]
            input.fill_(word_id)
        ...
        for i in range(100):
            ...
            # File write
            word = corpus.dictionary.idx2word[word_id]
            if word == '<eos>': return a #【mynote】2. 等到輸入結束的符號(eos)出現，再要求他開始真正輸出
            a.append(word)

def qa(path):
    with open(path, 'r', encoding='utf-8') as f: #【mynote】讀取一大串輸入
        for line in f:
            q = line.split()
            a = q2a(q)
            print(q, a) #【mynote】印出每一個答案
