...
def test(name):
    ...
    with torch.no_grad():
        with open(name+'_sample.txt', 'w', encoding='utf-8') as f:
            ...
            prob = torch.ones(vocab_size) #【mynote】預熱是指會先給一個詞作前導，引導第一個輸出的值
            input = torch.multinomial(prob, num_samples=1).unsqueeze(1).to(device) #【mynote】一開始隨便選一個詞作輸入

            for i in range(num_samples):
                # Forward propagate RNN 
                output, state = model(input, state) #【mynote】記憶
 
                # Sample a word id
                prob = output.exp() #【mynote】取得每一個字的輸出機率
                word_id = torch.multinomial(prob, num_samples=1).item() #【mynote】挑出最大機率的字作為預設輸出

                # Fill input with sampled word id for the next time step
                input.fill_(word_id) #【mynote】將預測輸出在塞回下一輪的輸入
