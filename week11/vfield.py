...
def div(f, p):
    ...
    for i in range(p.size):
        r += df(f, p, i)[i] #【mynote】散度就是將每一個軸偏微分後的值相加
    return r

# 旋度：函數 f 在點 p 上的旋度
# 限制：f 為 3 維向量函數 f(R3)->R3
def curl(f, p):
    rx = df(f, p, 1)[2]-df(f, p, 2)[1] #【mynote】旋度算出來還是一個向量，類似線性代數中的determine
    ry = df(f, p, 2)[0]-df(f, p, 0)[2] #【mynote】旋度只能在三維空間計算
    rz = df(f, p, 0)[1]-df(f, p, 1)[0]
    return np.array([rx, ry, rz])

# 拉普拉斯算子 ∆ = div∇ #【mynote】先取梯度(∇)再取散度(div)
def laplace(f, p):
